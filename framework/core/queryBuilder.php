            ...
            //UPDATE
             if(isset($this->parts['updateTable']) && isset($this->parts['set'])){
                ...
                foreach ($this->parts['set'] as $key => $value) {
                    //$setExpession = ($setExpession == '' ? '' : ',') . $key .'='. '\''. $this->dbcon->escape($value) .'\' ';
                ...
                //$builder->where('id=' . $db->escape($_REQUEST['id']));
...
//----------------------------------------- 
        ...
        //SELECT
            if(isset($this->parts['select']) && isset($this->parts['from'])){
               $this->sql = 'SELECT '. $this->parts['select'] . ' FROM '. $this->parts['from'] . $this->getWhere() ;
...
// https://github.com/BraTTik/MyHackerU/blob/QBsecond/php/mvc/core/QueryBuilder.php 